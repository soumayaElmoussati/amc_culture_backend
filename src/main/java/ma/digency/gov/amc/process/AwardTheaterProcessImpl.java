...
public class AwardTheaterProcessImpl implements AwardTheaterProcess{
    ...
    @Override
    public List<PriceTheaterResponse> getPriceDemandList() {
        List<DemandPrice> demandPriceList=demandPriceService.findAllDemandAwardTheater();
        List<PriceTheaterResponse> priceTheaterResponses=new ArrayList<>();
        for (DemandPrice demandPrice:demandPriceList) {
            PriceTheaterResponse priceTheaterResponse=new PriceTheaterResponse();
            priceTheaterResponse.setRefDemand(demandPrice.getRefDemand());
            priceTheaterResponse.setComment(demandPrice.getComment());
            priceTheaterResponse.setStatus(demandPrice.getStatus());
            priceTheaterResponse.setDecision_date(demandPrice.getDecision_date());
            priceTheaterResponse.setAwardCategories(attributionsPrixMapper.awardCategoryToAwardCategoriesResponse(demandPrice.getAwardCategories()));
            priceTheaterResponse.setOwnerPersonalInfoResponse(attributionsPrixMapper.artistAccountToOwnerPersonalInfoResponse(demandPrice.getAccountOwner()));
            priceTheaterResponse.setCreationDate(demandPrice.getDateCreation().toLocalDate());
            priceTheaterResponse.setTheaterPiece(demandPrice.getTheaterPiece());
            priceTheaterResponses.add(priceTheaterResponse);
        }
        return priceTheaterResponses;
    }

    @Override
    public List<PriceTheaterResponse> getPriceDemandListByRefArtist(String refArtistAccount) {
        List<DemandPrice> demandPriceList=demandPriceService.findAllDemandAwardTheaterByRefArtistAccount(refArtistAccount);
        List<PriceTheaterResponse> priceTheaterResponses=new ArrayList<>();
        for (DemandPrice demandPrice:demandPriceList) {
            PriceTheaterResponse priceTheaterResponse=new PriceTheaterResponse();
            priceTheaterResponse.setRefDemand(demandPrice.getRefDemand());
