...
public class AwardBookProcessImpl implements AwardBookProcess {
    ...
    @Override
    public List<PriceBookResponse> getPriceDemandList() throws ParseException {
        List<DemandPrice> demandPriceList=demandPriceService.findAllDemandAwardBook();
        List<PriceBookResponse> priceBookResponseList=new ArrayList<>();
        SimpleDateFormat formatter = new SimpleDateFormat("yyyy-dd-MM");
        for (DemandPrice demandPrice:demandPriceList) {
            PriceBookResponse priceBookResponse=new PriceBookResponse();
            priceBookResponse.setRefDemand(demandPrice.getRefDemand());
            priceBookResponse.setComment(demandPrice.getComment());
            priceBookResponse.setStatus(demandPrice.getStatus());
            priceBookResponse.setDecision_date(demandPrice.getDecision_date());
            priceBookResponse.setCreationDate(demandPrice.getDateCreation().toLocalDate());
            priceBookResponse.setAwardCategories(attributionsPrixMapper.awardCategoryToAwardCategoriesResponse(demandPrice.getAwardCategories()));
            priceBookResponse.setOwnerPersonalInfoResponse(attributionsPrixMapper.artistAccountToOwnerPersonalInfoResponse(demandPrice.getAccountOwner()));
            priceBookResponse.setBookPrice(demandPrice.getBookPrice());
            priceBookResponseList.add(priceBookResponse);
        }
        return priceBookResponseList;
    }

    @Override
    public List<PriceBookResponse> getPriceDemandListByRefArtist(String refArtistAccount) throws ParseException {
        List<DemandPrice> demandPriceList=demandPriceService.findAllDemandAwardBookByRefArtistAccount(refArtistAccount);
        List<PriceBookResponse> priceBookResponseList=new ArrayList<>();
        SimpleDateFormat formatter = new SimpleDateFormat("yyyy-dd-MM");
