...
import ma.digency.gov.amc.service.attributionsprix.AwardHonoraryService;
import ma.digency.gov.amc.service.attributionsprix.CandidateHonoraryAwardService;
import ma.digency.gov.amc.service.attributionsprix.DemandAHService;
...
public class AwardHonoraryProcessImpl implements AwardHonoraryProcess {
    ...
    @Override
    public List<PriceHonoraryResponse> getPriceDemandList() {
        List<DemandPrice> demandPriceList=demandPriceService.findAllDemandAwardHonorary();
        List<PriceHonoraryResponse> priceHonoraryResponses=new ArrayList<>();
        for (DemandPrice demandPrice:demandPriceList) {
            PriceHonoraryResponse priceHonoraryResponse=new PriceHonoraryResponse();
            priceHonoraryResponse.setRefDemand(demandPrice.getRefDemand());
            priceHonoraryResponse.setComment(demandPrice.getComment());
            priceHonoraryResponse.setStatus(demandPrice.getStatus());
            priceHonoraryResponse.setDecision_date(demandPrice.getDecision_date());
            priceHonoraryResponse.setAwardCategories(attributionsPrixMapper.awardCategoryToAwardCategoriesResponse(demandPrice.getAwardCategories()));
            priceHonoraryResponse.setOwnerPersonalInfoResponse(attributionsPrixMapper.artistAccountToOwnerPersonalInfoResponse(demandPrice.getAccountOwner()));
            priceHonoraryResponse.setCreationDate(demandPrice.getDateCreation().toLocalDate());
            priceHonoraryResponses.add(priceHonoraryResponse);
        }
        return priceHonoraryResponses;
    }

    @Override
    public List<PriceHonoraryResponse> getPriceDemandListByRefArtist(String refArtistAccount) {
        List<DemandPrice> demandPriceList=demandPriceService.findAllDemandAwardHonoraryByRefArtistAccount(refArtistAccount);
        List<PriceHonoraryResponse> priceHonoraryResponses=new ArrayList<>();
        for (DemandPrice demandPrice:demandPriceList) {
            PriceHonoraryResponse priceHonoraryResponse=new PriceHonoraryResponse();
            priceHonoraryResponse.setRefDemand(demandPrice.getRefDemand());
